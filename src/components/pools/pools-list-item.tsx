...
import { POOL_STATUSES_CONFIGS, POOLSTATUS } from '@/types/pools'
...
export default function PoolListCard({
  ...
  const dateString = getStatusString({ status, startDate, endDate })
  ...
  return (
    <Link
      href={`/pool/${id}`}
      onMouseEnter={() => prefetchPool(id)}
      onFocus={() => prefetchPool(id)}

...
import { DateTimeRange } from '@/components/shared/forms/date-time-range.control'
import { ImageUploader } from '@/components/shared/forms/image-uploader.control'
import type { MultiSelectValue } from '@/components/shared/forms/multi-select.control'
import { MultiSelect } from '@/components/shared/forms/multi-select.control'
import type { NumberValue } from '@/components/shared/forms/number.control'
import { Number } from '@/components/shared/forms/number.control'
import type { TextAreaValue } from '@/components/shared/forms/text-area.control'
import { TextArea } from '@/components/shared/forms/text-area.control'
import { Text } from '@/components/shared/forms/text.control'
import type { UrlValue } from '@/components/shared/forms/url.control'
import { Url } from '@/components/shared/forms/url.control'
...
export default function CreatePoolPage() {
    ...
    const createPoolServerMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            // TODO: probably better to redeirect to the new created pool page
    ...
    const handleFormSubmission = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        await new Promise(() => {
            setTimeout(() => {
                ...
                toast.success('Pool Created', { description: 'Pool has been created successfully' })
            }, 1000)
        })
    }
    ...
    return (
        <div className='mt-8 px-6'>
            <div className='text-center text-base font-medium leading-normal text-[#090909]'>Create Pool</div>
            <form className='flex w-full flex-col gap-6 py-6' onSubmit={e => void handleFormSubmission(e)}>
                <section className='flex flex-1 flex-col gap-6'>
                    {/* Banner Image */}
                    <div>
                        <Label className='text-base font-medium text-[#090909]'>Choose Image</Label>
                    ...
                    {/* Name */}
                    <div>
                        <Label className='text-base font-medium text-[#090909]'>Name of Pool</Label>
                        <p className='mb-4 mt-1.5 text-xs font-medium text-[#b2b2b2]'>Enter a name for your Pool</p>
                        <Text value={name} setValue={setName} />
                    </div>

                    {/* Main Host */}
                    <div>
                        <Label className='text-base font-medium text-[#090909]'>Name of Host</Label>
                        <p className='mb-4 mt-1.5 text-xs font-medium text-[#b2b2b2]'>Enter the host name</p>
                        <Text value={mainHost} setValue={setMainHost} />
                    </div>

                    {/* Co-Hosts  */}
                    <div>
                        <Label className='text-base font-medium text-[#090909]'>Add Co-Host</Label>
                        <p className='mb-4 mt-1.5 text-xs font-medium text-[#b2b2b2]'>Please add your Co-Host</p>
                        <MultiSelect value={coHosts} setValue={setCoHosts} />
                    </div>

                    {/* Date of Event */}
                    <div>
                        <Label className='text-base font-medium text-[#090909]'>Date of Event</Label>
                        <p className='mb-4 mt-1.5 text-xs font-medium text-[#b2b2b2]'>
                    ...
                    {/* Description */}
                    <div>
                        <Label className='text-base font-medium text-[#090909]'>Description</Label>
                        <p className='mb-4 mt-1.5 text-xs font-medium text-[#b2b2b2]'>
                    ...
                    {/* Soft Cap */}
                    <div>
                        <Label className='text-base font-medium text-[#090909]'>Soft Cap</Label>
                        <p className='mb-4 mt-1.5 text-xs font-medium text-[#b2b2b2]'>
                    ...
                    <div>
                        ...
                        <p className='mb-4 mt-1.5 text-xs font-medium text-[#b2b2b2]'>Paste a link to your rules</p>
